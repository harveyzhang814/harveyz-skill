...
□ 6. 模式 B 的每条归因签名都有体裁基线对照记录？
□ 7. 每条签名都有 ≥1(描述)/≥2(归因)逐字定位实例？
□ 8. 没有把假设写成事实的措辞（"作者认为" vs "本文指向一个…倾向"）？
□ 9. 每条归因签名都跑过独立对抗 pass 和留出验证？
□ 10. 每条归因签名都列了仍存活的对立解释，且置信度已减去它们？
□ 11. 没有出现"清单扫描"痕迹（见失败模式 F1）？
□ 12. 全篇是否声明了认识论上限（单篇 / 隐含作者 / 假设）？
```
