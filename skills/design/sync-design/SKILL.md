...
完成后按本次模式路由：跳转至[设计阶段](#设计阶段)或继续[同步流程](#流程strict--不可跳过或合并阶段)。
...
3. 全部处理完毕后，写入 manifest 后按本次模式路由：跳转至[设计阶段](#设计阶段)或继续[同步流程](#流程strict--不可跳过或合并阶段)。
...
**否：** 保持现有路径，按本次模式路由：跳转至[设计阶段](#设计阶段)或继续[同步流程](#流程strict--不可跳过或合并阶段)。
