...
先读取 `references/stacks.md`，然后依次执行：
...
`styleStrategy` 取值：`tailwind` / `css-modules` / `vanilla-extract` / `styled-components` / `emotion` / `unocss` / `scss` / `css` / `unknown`。多种混用时记录主要策略，其余在 `notes` 中说明。
...
写入 `.hskill/sync-design/manifest.json`：
...
5. 输出迁移摘要：
   ```
   迁移完成：旧路径 <outputDir> → .hskill/sync-design/html（N 个文件）
   ```
