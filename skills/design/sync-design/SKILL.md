...
**如果文件存在：**
检查 `config.outputDir` 是否以 `.hskill/` 开头：
- **否**（旧路径）→ 执行【附录 C：旧路径自动迁移流程】，迁移完成后继续。
- **是** → 直接继续。

然后：
- `entries` 为空 → 执行【附录 B：已有 HTML 扫描流程】，完成后继续步骤 ②。
- `entries` 非空 → 直接继续步骤 ②。
...
1. 创建目标目录：
   ```bash
   mkdir -p .hskill/sync-design/html
   ```
2. 将旧目录下所有 `*.html` 文件移动到新目录：
   ```bash
   mv <outputDir>/*.html .hskill/sync-design/html/ 2>/dev/null
   ```
3. 更新 manifest 内容：
   - `config.outputDir` → `.hskill/sync-design/html`
   - 所有 `entries[].htmlFile`：将旧路径前缀替换为 `.hskill/sync-design/html`
4. 写回 manifest。
5. 询问是否删除旧目录（仅当旧目录已清空时）：
   ```bash
