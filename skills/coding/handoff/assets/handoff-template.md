...
**交接源节点**：<nodeId> · <title> <!-- 仅当 author 在 Agent Canvas 画布节点里时写这一行，否则整行删掉；author 已当场建过关系时在末尾注明"（hands-off-to 已建）" -->
...
>
> **开工前**：若上面有「交接源节点」一行、且你也在 Agent Canvas 画布节点里，先建一条 `hands-off-to` 关系（`agent-canvas-ctl whoami` 取自己的 id，核对 `node-relations` 里没有后 `link-nodes --from <交接源节点> --to <自己> --type hands-off-to`）。**只建这一条**——别建 `implements`、别另立需求节点。
...
5. 若在 Agent Canvas 画布节点里，按 `SKILL.md` author 第 5 步补上抬头的「交接源节点」行（并在已能指认接手节点时当场建 `hands-off-to`）。
6. 跑 `SKILL.md` 里的完整性门禁。
