...
- **工作流样板** `workflow`：注入交接文档 §6 的项目规范（分支/worktree/hooks/用哪些 skill）。
- **验证工具** `verification`：注入 §8 的项目验证约定（跑什么测、要不要真产物 E2E）。
- **权威依据指向** `authority`：起草 §1/§5 时自动挂链的**可移植权威**位置（spec 目录、架构文档）。不含 memory——交接文档不引 memory（可能陈旧、接手方访问不到）。
