...
`workflow` 只写项目不变量与宿主分支。不要把某一宿主的工具伪装成通用步骤；若进入工作区的
方式不同，分别写 Claude Code、Codex 与通用兜底。handoff 文档可用
`workspace_mode: same-workspace|shared-worktree`；旧文档省略时，由 `branch` / `worktree` 是否存在推断。
可选 `base_commit` 只记录完整的 40 位分叉基线，不代表文档或最终改动的当前提交。
