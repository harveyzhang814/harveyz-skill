...
**首先输出 Mermaid flowchart**，覆盖完整主流程和所有已知条件分支。节点使用简短标签，条件判断用菱形 `{condition}` 表示，分支边标注「是/否」或具体条件文字。
...
- **入口与触发**：什么操作或上下文启动这个 skill？触发条件是什么？
- **主流程步骤**：每一步做了什么？产出是什么？被下一步怎么使用？
- **条件分支**：逐一列出所有判断点，用「如果…则…否则…」格式说明每条路径的执行差异
