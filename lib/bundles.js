...
const HSKILL_TOOLS_DATA_OLD = path.join(os.homedir(), '.local', 'share', 'hskill', 'tools')
const HSKILL_TOOLS_DATA     = path.join(os.homedir(), '.hskill', 'tools')

// One-time silent migration: move old tools metadata dir to new location
;(function _migrateToolsData() {
  try {
    if (fs.existsSync(HSKILL_TOOLS_DATA_OLD) && !fs.existsSync(HSKILL_TOOLS_DATA)) {
      fs.mkdirSync(path.dirname(HSKILL_TOOLS_DATA), { recursive: true })
      fs.renameSync(HSKILL_TOOLS_DATA_OLD, HSKILL_TOOLS_DATA)
    }
