  ...
  for (const { toolName, srcPath } of tools) {
    ...
    if (destExists && !force) {
      if (!process.stdout.isTTY) {
        ...
        console.log(chalk.dim(`  · Skipped ${toolName} (already exists — use --force to overwrite)`))
        continue
      }
    ...
    for (const { skillName, srcPath, version: availableVersion } of skills) {
      ...
      if (destExists && !force) {
        ...
        if (availableVersion && installedVersion === availableVersion) {
          ...
          console.log(chalk.dim(`  · Skipped ${skillName} (up-to-date ${installedVersion})`))
          continue
        }
        ...
        if (!process.stdout.isTTY) {
          ...
          console.log(chalk.dim(`  · Skipped ${skillName} (outdated ${installedVersion} → ${availableVersion ?? '—'}, use --force to overwrite)`))
          continue
        }
