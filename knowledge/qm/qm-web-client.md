...
> 补读了渲染安全这一条线：`src/markdown-sanitize.ts`、`src/marked-dedupe.ts`、
> `server/index.ts` 的两份 CSP，以及四个相关测试文件（§6）。
...
## 六、渲染层：净化器只有 16 行，因为它不是第一层

转录里渲染的是模型输出和工具输出——**内容来源取决于 agent 这一轮读了什么**：一个网页、一封邮件、一个仓库里的文件。所以这里的 markdown 不是「用户自己写的」，是结构上不可信的。注入方能让模型吐出什么，就能让浏览器渲染什么，而渲染发生的位置正是那个持有登录态的页面。

```mermaid
flowchart TD
    A["模型/工具输出<br/>（不可信）"] --> B["marked 解析<br/>第三方组件 markdown-block 调用"]
    B --> C["DOMPurify postprocess<br/>第二层：内容长什么样"]
    C --> D["浏览器渲染"]
    D --> E["CSP script-src 'self'<br/>第一层：能不能执行"]
    F["agent 产物文件<br/>预览 / 下载"] --> G["sandbox CSP<br/>第三层：算不算同源"]
    G --> D
    H["远端 img src / style url()<br/>两层都没关"] -.-> D
    style A fill:#7f1d1d,color:#ffffff
    ...
    style F fill:#7f1d1d,color:#ffffff
    style G fill:#166534,color:#ffffff
    style H fill:#78350f,color:#ffffff
```

### 6.1 那 16 行里没有净化逻辑

它做的是**接线**：把 DOMPurify 挂成 marked 的 `postprocess` 钩子，加一个标志防重复安装。真正的过滤在 DOMPurify 里。所以「16 行给一整个转录用」这个比例不成立——它不是一个手写净化器。我原来的警觉搞错了对象。

值得看的是它挂在哪。渲染 markdown 的不是自己的代码，是第三方组件 `<markdown-block>`（来自 `@mariozechner/mini-lit`），**那个组件不接受净化器参数**。这里的做法是不改组件，改组件脚下的东西——marked 是单例、`postprocess` 是公开钩子，于是全进程所有走 marked 的渲染一次性都被套上。

代价两条，都不小：

- **依赖模块同一性。** 打包后如果存在两份 marked（比如 mini-lit 里嵌了自己那份），组件用的就是没打补丁的那份，净化**静默失效**——不报错、不少任何东西，只是不过滤了。仓库里确实有一个测试从 mini-lit 的角度去解析 marked（`test/markdown-tilde.test.ts`），**但它断言的是波浪号不变成删除线，不是「这两个 marked 是同一个对象」**。护栏立在旁边，方向差了一点。
- **同一个手法被用了第二次，为了完全不同的事。** `marked-dedupe.ts` 又打了一次补丁，包住 `marked.use` 把重复注册的扩展丢掉——说明那个第三方组件会反复注册扩展、列表会一直涨。**两次都是「改不了组件，就改它站着的地板」**，两次都靠 `chat.ts` 顶部的 import 顺序生效，而这个顺序没有任何注释保护（一个按字母排序的 lint 规则就会把它换掉）。

### 6.2 真正的第一层是 CSP

服务端给 SPA 的每个响应都带一份内容安全策略：脚本只能同源、禁 inline、禁 eval，`object-src` 和 `base-uri` 关死，表单只能提交回同源。

这条决定了严重性的量级——**就算 DOMPurify 被绕过，注进来的 `<script>` 或 `onerror=` 也不会执行**，因为没有 `unsafe-inline`。净化器是第二层。一个 16 行的接线文件配一份真 CSP，比一个三百行的手写净化器配一份带 `unsafe-inline` 的 CSP 安全得多。

唯一放宽的一格是 `'wasm-unsafe-eval'`：允许编译 WebAssembly，不打开 JS 的 eval。**按需要放开最小的那一格，而不是整个 `unsafe-eval`。**

`style-src` 留了 `unsafe-inline`，这是 Lit 和 KaTeX 的硬要求，绕不过去。后果在下一节。

### 6.3 两层都没关的那道缝：不需要 JS 的外带

我按仓库里那份配置实测了一遍 DOMPurify（跑出来的，不是读文档）。已知花样都挡住了：`annotation-xml`——MathML 里的 HTML 集成点，历史上变异型 XSS 的经典入口——被删，SVG 的 `foreignObject` 被删，外部 `<use>` 被删，`<animate>` 改 href 被删。**加进白名单的是 `annotation` 而不是 `annotation-xml`，加对了那一个**：前者装的是 KaTeX 留的 TeX 源码（为了让人能复制原式），内容是纯文本。

留下来两样东西：

1. **`<img src="https://任意站点/…">` 原样通过。** CSP 的 `img-src` 里有 `https:`，全网放行。
2. **`style="background:url(https://任意站点/…)"` 原样通过。** `unsafe-inline` 允许行内样式，取图地址仍归 `img-src` 管，还是全网放行。

两条都是**页面一渲染就自动发出去的请求，不需要点击，也不需要执行一行 JS**。注入方要做的只是让模型在输出里带上一个图片地址，把想带走的东西编进 URL。`referrer-policy: no-referrer` 挡的是 referer 头，挡不住路径本身。

这不是疏漏，是权衡：**`img-src https:` 换来的是模型可以在回答里贴图。** 收紧成白名单或走服务端代理，就得接受贴图要么不显示、要么慢一拍、要么多一个代理组件要运维。分歧点在于「转录里出现远端图片」在产品上有多常见——代码里看不出来。

顺带一个反例，说明这份 CSP 确实在干活：`<form action="https://外站" target="_blank">` 能过净化器，但 `form-action 'self'` 会在提交那一刻拦下。**同一份策略里，一条指令兜住了净化器放过去的东西，另一条（`img-src`）没兜住。**

### 6.4 agent 产物走的是第三套：撤销同源

转录之外还有一条路——预览/下载 agent 生成的文件，以及部署预览。这两个响应带的不是 SPA 那份策略，是一份只有 `sandbox` 指令的策略：允许脚本、表单、弹窗、下载，**唯独不给 `allow-same-origin`**。

这一格的缺席就是全部意义：内容虽然从同一个域发出来，浏览器会把它放进一个不透明来源，拿不到父页面的 cookie 和 localStorage。**「同源」被响应头单方面撤销了**，不需要把用户文件挪到另一个域名去托管（那是同类问题的常见解法，代价是多一套证书、多一条部署路径）。

配套还有两处。净化器的测试里有一条向量正是 `<iframe src=/api/files/x/content>`——**他们想到了「用 iframe 把 agent 产物嵌回主页面」这条路，在净化器上也堵了一遍**。以及会话 cookie 是 HttpOnly，所以即便真执行了脚本，令牌本身偷不走；能做的是以用户身份发请求，那是另一个量级的事。

### 6.5 判断与把握度

**技术判断：** 这一节值得抄的不是任何一条具体指令，是**层次的划分方式**——能不能执行交给 CSP，长什么样交给净化器，算不算同源交给沙箱指令。三层各自可以独立失效而不塌。反过来说，如果一个前端的 XSS 防线只有净化器一层，那份净化器的代码量确实该引起警觉；我最初的方向没错，这里的答案只是「另外两层在」。

**产品判断（依据只到代码，未考虑定价与用户分层）：** `img-src https:` 是这三层里唯一一处为体验让掉的安全。在一个专门跑 agent、而 agent 专门读外部内容的产品里，它同时也是最容易被利用的一格。

**把握度：**

- **读到：** 净化配置、SPA 与沙箱两份 CSP、cookie 标志、`chat.ts` 的安装点与 import 顺序、四个相关测试文件。
- **跑出来：** 那十条向量的实际输出，用仓库里那份配置、同一个大版本的 DOMPurify（`^3.4.12` / 实测 3.4.13）在隔离目录里跑的。
- **推出来：** 「mini-lit 用的是同一个 marked 单例」。`node_modules` 没装（上游仓库只读，我不往里装东西），这一条是从「他们特地把 `target` 加进白名单、又特地写了 dedupe 补丁」反推的——两件事都只有在组件确实走全局 marked 时才有意义。**这一条如果错了，整个第二层就是失效的。**
- **没查：** mini-lit 组件内部有没有自己的 innerHTML 写入路径（那会绕过 `postprocess`）；打包产物里 marked 的实例数；`REQUIRE_SIGNED_PORTAL_IDENTITY` 的部署侧默认（与本节无关，仍未查，见 §7-6）。

**代码锚点：** `plugins/web-ui/src/markdown-sanitize.ts:4-16`（配置与安装）、`src/marked-dedupe.ts:6-17`（第二次打补丁）、`src/chat.ts:5-6`（import 顺序）与 `:104`（安装点）、`server/index.ts:129-153`（SPA CSP 与安全头）、`:155`（沙箱 CSP）、`:1121` 与 `:1884`（两处使用）、`test/markdown-sanitize.test.ts:17-50`（向量表）、`test/markdown-tilde.test.ts:6-7`（从 mini-lit 视角解析 marked）。
...
我读的是服务端和桥。原先把 `markdown-sanitize.ts`（16 行）标成本篇最大未查项，**现已查完，见 §6**：那 16 行是接线不是净化，真正的第一层是 CSP。剩下 55 个组件仍未读，但已知渲染入口只有 `<markdown-block>` 一个，风险面比原先估计的窄。
...
1. ~~**前端 55 个文件里的 markdown 净化没看，是本篇最大未查项。**~~ **已查完，见 §6。** 仍未读的是 55 个组件本身；已知的渲染入口只有 `<markdown-block>` 一个，但我没排除组件内部有别的 innerHTML 写入点。
2. **「mini-lit 用的是同一个 marked 单例」是推的，不是读到的**（§6.5）。`node_modules` 未安装，我不往上游仓库里装东西。这一条错了，整个第二层就是失效的——是本节唯一一处「错了会很严重」的推断。
3. **`REQUIRE_SIGNED_PORTAL_IDENTITY` 的部署侧默认没查**（§7-6）。这条直接决定第三节那套的实际有效性，是本篇最重要的未查项。
4. **`/v1/runs` 和 `/v1/runs/:id` 在 `USER_SCOPED` 里没有 `field`**，意味着 core 从 portal 身份推导行动者而不是比对请求字段。我确认了它们在表里、也确认了闸的代码，但**没有去读 `getRun` / `getActiveRunForThread` 两个处理函数**，所以「core 确实按这个人过滤」这一步我是从分类表推的，不是从处理逻辑读到的。
5. **「模型选择器只是偏好」在界面上是否表达清楚，没看。** README 说清楚了，UI 上未必。
6. **`SYSTEM` 表 32 条我没有逐条核对是否都该在里面。** 一条本该 user-scoped 的路由被误放进 `SYSTEM`，就会绕过身份比对，而且不会有任何东西报错——§3.3 那个漂移测试守的是反方向，挡不住这种误放。
...
17. **分三层，各管一件事：能不能执行交给 CSP，长什么样交给净化器，算不算同源交给沙箱指令。** 三层可以独立失效而不塌。判断一份净化器「够不够」之前，先问它是第几层。
18. **净化器代码量本身不说明问题，它挂在哪、挡在第几道才说明问题。** 16 行接线 + 一份禁 inline 的 CSP，强于三百行手写净化器 + 一份带 `unsafe-inline` 的 CSP。
19. **给第三方组件加安全行为的办法是打补丁到它脚下的共享库**（这里是 marked 的 `postprocess` 钩子），前提是那个库是单例且有公开钩子。代价是**依赖模块同一性**——打包出两份就静默失效，所以必须有一个断言「组件用的和我打补丁的是同一个对象」，而不只是断言渲染结果。
20. **对不可信内容撤销同源，用响应头上的 `sandbox` 指令、并且不给 `allow-same-origin`**，不必把用户产物挪到另一个域名托管（后者要多一套证书和部署路径）。
21. **XSS 之外还有一条不需要执行 JS 的外带通道：自动加载的子资源。** `<img src>` 和 CSS 的 `url()` 在页面渲染时自动发出请求，净化器通常不拦，`img-src` 放开 `https:` 的 CSP 也不拦。要不要收紧，取决于「远端图片」在产品里的实际价值。
22. **CSP 只放开需要的那一格。** 这里是 `'wasm-unsafe-eval'` 而不是整个 `unsafe-eval`。

**关于安全默认值**

23. **一个不安全的默认（`maxBytes = Infinity`），最好的补救不是「记得传参数」，是改名导入 + 本地包一层，让不安全的那个在你的文件里叫不出名字。** 这个仓库三个插件三种处理，只有这一种不依赖纪律。

**关于产品边界**

24. **宁可少一个功能，也不要一个会骗人的功能。** Web 不能回复 Slack 线程，因为那条回复对 Slack 参与者不可见——半吊子的双向投影比没有更糟。
25. 砍掉功能时要给出**设计过的替代路径**，而不是一句「暂不支持」。这里是「在同一个工作区开一个新的 Web 对话」，它顺带把「共享的是工作区，不是对话」这个事实变成了界面上看得见的结构。
26. **权限分级除了按「权限大小」，还可以按「做错了会不会有人发现」。** 往公开频道发消息全频道可见（社会性约束在起作用），从 Web 挂载那个频道的工作区不可见——所以后者门槛更高，尽管前者「动静」更大。
...
- **[[qm-crosscutting]] 的「不安全默认值」主题在本篇有第三个样本。** 那篇讲 `swallow` 与 `readBody`，本篇 §6.1 是同一个形状的另一面：不是「默认不安全所以要记得改」，而是「第三方组件不给参数所以只能改它脚下的库」——两者都要一个断言来兜住纪律靠不住的部分。
- **未覆盖的只剩前端组件**（55 个文件，约 15,400 行）和 `cli/`（109 文件）、`scripts/`（46 个）、`skills-seed/`（79 文件）。`src/` 与 `plugins/` 的服务端部分至此全部读过；前端只补了渲染与安全策略这一条线（§6）。
