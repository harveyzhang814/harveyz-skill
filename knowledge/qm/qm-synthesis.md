...
**这条规则的形状在同一个仓库里至少出现了六次**，其中一半是真的违反（以下行号本次全部重新验证）：
...
> **判据补记（见 [[qm-web-client]] §4）：** 上面这个划分是逐个判出来的。有一句可以直接套的话——**假设这个 Map 下一毫秒被清空，会发生什么。** 答案是「多花点时间重算」→ 缓存，放内存没问题；答案里出现「有人被拒绝／被放行／某件事不会发生了」→ 状态，必须持久化或把非持久性暴露出来。
> 那一篇还给出了本节之外的**第二个结构性答案**：不是把持久性暴露到接口上，而是让本地索引命中就省一次往返、不命中就问权威源——于是它只能省工作，不能授予权限。
...
3. **那六处里，`src/sandbox/` 的容器引用计数我这次没有重新定位行号**，它来自 [[qm-execution-layer]] 当时的分析。其余五处本次全部重新验证。
