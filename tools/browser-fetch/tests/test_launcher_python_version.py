...
实测过的失败链：PATH 上 /usr/bin/python3（macOS 自带 3.9.6）排在 homebrew
前面时，launcher 用它建 venv → `pip install` 因 requires-python >=3.11 失败 →
venv 里没有 browser-fetch 入口。此后即使用正确的 python3 重跑也修不好：
`python3 -m venv` 不带 --clear 不会替换已存在的解释器，venv 永远停在 3.9，
而报出来的错是 `No module named playwright` —— 完全指不到真原因。

跟 test_launcher_stdout_contract 一样做静态断言：真跑 bootstrap 需要
`playwright install chromium` 的真实下载，不能作为测试前置。这里钉住的是
"两个分支的 venv 创建都带 --clear、且前面都有版本校验"这个语法事实；
覆盖不到的是校验逻辑本身在 3.9 上是否真的退出（那要一个 3.9 解释器才能测）。
"""
import re
