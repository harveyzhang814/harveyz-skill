...
每条观察带时间戳和依据来源：三个月后看到「他擅长 X」，得能判断这是基于 40 条
推文写的，还是基于 3 个视频标题猜的。时间戳可以只到日期，也可以带时分——人工
一天记好几条时需要分先后。依据来源同时就是作者标记：`人工` 表示是本人下的判断。
"""
import re
from pathlib import Path

_OBS_RE = re.compile(r"^### (\d{4}-\d{2}-\d{2}(?: \d{2}:\d{2})?) · 依据：(.*)$")
