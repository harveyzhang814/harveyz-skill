...
7. Optional but useful: temporarily force `_xcom_scrape(url, pw_cookies, headless=False)` to raise (e.g. add `raise Exception("test")` right after `browser = await p.chromium.launch(...)`, temporarily) to confirm the headless fallback path also runs and returns *some* content (even if lower quality) rather than crashing — then revert that temporary change.

Report back what you found — if the headed window didn't appear, if content looked wrong, or if anything raised an unexpected error, that's a real finding to fix before this capability is considered working, not something to silently accept.
