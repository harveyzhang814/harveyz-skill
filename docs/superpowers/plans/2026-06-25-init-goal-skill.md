# init-goal Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create the `init-goal` skill — a guided dialogue that generates structured `/loop` prompt files for goal-driven automation.

**Architecture:** A single `SKILL.md` in `skills/meta/init-goal/` contains the complete dialogue instructions for Claude, including all 5 built-in templates and output format specs. Skills-index.json is updated to register it. A bats test validates the SKILL.md structure.

**Tech Stack:** Markdown (SKILL.md), JSON (skills-index.json), bats (tests)

## Global Constraints

- SKILL.md frontmatter must have: `name`, `version` (semver string in quotes), `description`, `user_invocable: true`
- Skill path: `skills/meta/init-goal/`
- Data directory: `~/.hskill/init-goal/<goal-slug>/`
- Output files: `prompt.md` (static), `log.md` (appended per round), `summary.md` (generated at exit)
- `goal-slug` is kebab-case derived from the goal text (lowercase, spaces→hyphens, strip punctuation)
- All dialogue is one question per message
- Spec: `docs/superpowers/specs/2026-06-25-init-goal-design.md`

---

### Task 1: Create `skills/meta/init-goal/SKILL.md`

**Files:**
- Create: `skills/meta/init-goal/SKILL.md`

**Interfaces:**
- Produces: `~/.hskill/init-goal/<goal-slug>/prompt.md` at runtime

- [ ] **Step 1: Create the skill directory**

```bash
mkdir -p skills/meta/init-goal
```

- [ ] **Step 2: Write `SKILL.md`**

Create `skills/meta/init-goal/SKILL.md` with the following complete content:
...
```bash
ls skills/meta/init-goal/SKILL.md
```
...
```bash
git add skills/meta/init-goal/SKILL.md
git commit -m "feat(init-goal): add SKILL.md with guided dialogue and 5 templates"
```

---

### Task 2: Register in `skills-index.json`

**Files:**
- Modify: `skills-index.json`

**Interfaces:**
- Consumes: `skills/meta/init-goal/SKILL.md` from Task 1
- Produces: `init-goal` visible to `hskill install` and test suite

- [ ] **Step 1: Add skill entry to `skills-index.json`**

Open `skills-index.json`. In the `"skills"` array, add the following entry (alphabetically within the `meta` group, after `init-workflow` if present, otherwise after the last `meta/` entry):
...
- [ ] **Step 2: Update `bundleMeta.meta` description**

In `skills-index.json`, find `"bundleMeta"."meta"` and append `+ init-goal` to the skill list:

Before:
```json
"meta": "元操作工具（clean-git + archive-skill + dedup-skill + contribute-skill + publish-skill + release-project + runby-opencode）"
```

After:
```json
"meta": "元操作工具（clean-git + archive-skill + dedup-skill + contribute-skill + publish-skill + release-project + runby-opencode + init-goal）"
```

- [ ] **Step 3: Run the existing skill format test to verify**

```bash
npm test 2>&1 | grep -E "init-goal|PASS|FAIL|ok |not ok"
```

Expected: `init-goal` appears in passing checks, no `not ok` lines mentioning `init-goal`.

- [ ] **Step 4: Commit**

```bash
git add skills-index.json
git commit -m "chore(init-goal): register in skills-index.json under meta bundle"
```

---

### Task 3: Add bats structural tests

**Files:**
- Create: `skills/meta/init-goal/tests/init-goal.bats`

**Interfaces:**
- Consumes: `skills/meta/init-goal/SKILL.md` from Task 1
- Produces: test coverage for SKILL.md structure

- [ ] **Step 1: Create test directory**

```bash
mkdir -p skills/meta/init-goal/tests
```

- [ ] **Step 2: Write `init-goal.bats`**
...
```bash
bats skills/meta/init-goal/tests/init-goal.bats
```

Expected output: all tests show `ok`, no `not ok` lines.
...
Expected: all existing tests still pass, new init-goal tests included.

- [ ] **Step 5: Commit**

```bash
git add skills/meta/init-goal/tests/init-goal.bats
git commit -m "test(init-goal): add bats structural tests for SKILL.md"
```
...
|---|---|
| 对话式引导，8 步流程 | Task 1 (SKILL.md) |
| 模版选择（Step 1）+ 5个模版 | Task 1 (SKILL.md) |
| 目标 / 执行 / 评估 / 约束 / 退出条件各一问 | Task 1 (Steps 2-6) |
| 退出条件：明确 + 兜底两子问题 | Task 1 (Step 6) |
| 汇总确认 + 可修改字段 | Task 1 (Step 7) |
| goal-slug 生成规则 | Task 1 (Step 8) |
| 写入 prompt.md（静态） | Task 1 (Step 8) |
| log.md 每轮条目格式 | Task 1 (prompt.md 中的过程文档段落) |
| summary.md 格式 | Task 1 (prompt.md 中的过程文档段落) |
| 三文件路径 `~/.hskill/init-goal/<slug>/` | Task 1 (Step 8) |
| 向用户展示启动命令 | Task 1 (Step 8) |
| 注册到 skills-index.json | Task 2 |
| 结构测试 | Task 3 |

**Placeholder scan:** No TBD/TODO in any task. All code blocks complete.

**Type consistency:** No cross-task type dependencies (pure Markdown/JSON/bash).
