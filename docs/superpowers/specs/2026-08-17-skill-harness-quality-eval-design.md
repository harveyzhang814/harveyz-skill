...
| hermes 的产出物是否落在 jail 内 | 已查，实测成立。真实跑一格 `research/extract-url@hermes/native`，agent 用标准 `os.path.expanduser("~")` 写文件，落在 jail 内并被 harvest 正确捞到（`artifacts/.hskill/harness-probe/hermes-artifact-check.md`，内容与要求一致，`harvestErrors: []`）。见 [measurements/2026-08-17-quality-eval-e2e.md#step-6-hermes-产出物是否落在-jail-内补测2026-08-17](measurements/2026-08-17-quality-eval-e2e.md) | `hermesProfile.artifactChannel: 'jail'` 成立，无需像 pi 那样降级；不能把 pi 认证失败的教训类推到 hermes——两边 isolation 设计不同，须分别实测 |
