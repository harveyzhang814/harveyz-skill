...
| 项 | 所在 | 风险 |
|---|---|---|
| ~~68 个 `meta.json` 中 50 个字段并存的成因~~ **已实测（2026-09-15）** | §1.2 | 已排除：见下方结论 |
| ~~YouTube handle 是否大小写不敏感~~ **已实测（2026-09-15）** | §2.3 | 已排除：见下方结论 |
...
**50 个文件字段并存的成因**：不是未发现的第三个持续写入方。是 `scripts/migrate-store.sh`
（commit `04bdf75`，2026-09-02 那次一次性存储迁移）里"视频 meta.json 回填"这段内嵌 Python
（该文件第 406–478 行）。跟 vdl 和 `archive.py` 不同，这段脚本**显式做的是并入不是覆盖**
（第 459–469 行的读现有文件、逐 key 补空位，注释原话："回填必须是并入而不是覆盖，否则重跑
一次就把那些抹掉了"）。它已经跑过、已经提交，此后不会再自动触发——不是一个仍在运行、会继续
影响新任务的写入方。

实测核对（`~/knowledge/videos/work/*/meta.json`，68 个文件）：both=50、archive_only=16、
vdl_only=2、neither=0，与 spec §1.2 表格吻合。抽查 3 个 both 样本，字段集合正是 vdl 21 个 +
archive 10 个字段的并集去重，与迁移脚本的合并逻辑一致，不是巧合。

**结论对实现的影响**：现存、会持续运行的写入方还是只有两个（vdl `finalizeTaskMeta` 与
`archive.py`），两者都是全量覆盖，§2.1 那三个新增字段若只改一处会被后写者抹掉，
必须两处都改——这正是 spec §1.2 / 关键决定 #2 已经定的方案，不需要改设计，只是排除了
"有第三个我们不知道的写入方"这个风险。

**YouTube handle 大小写不敏感**：已用 Google 官方帮助文档核实（非推断）。YouTube 官方说明
明确写着 handle 不区分大小写，`@TingHu888` 与 `@tinghu888` 指向同一频道，且 handle 全平台
唯一。来源：[Learn about YouTube handles](https://support.google.com/youtube/answer/11585688)
（Google 官方支持页）。归一规则（去 `@`、转小写）可以按 spec §2.3 原样实现，无需改动。
