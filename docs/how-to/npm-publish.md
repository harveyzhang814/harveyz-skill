...
```bash
npm version patch --no-git-tag-version   # bug fix / 小改动 → 0.1.2 → 0.1.3
npm version minor --no-git-tag-version   # 新增 skill → 0.1.2 → 0.2.0
npm version major --no-git-tag-version   # 破坏性变更 → 0.1.2 → 1.0.0
```

在 chore/bump-x.x.x 分支提交，merge 到 staging 后手动打 tag：
